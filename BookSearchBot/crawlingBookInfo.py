...
def getSearchResult():
    elem = driver.find_elements_by_css_selector('div.ikc-search-result-listup > p')
    if len(elem) == 0:  # 검색 결과 없음
        return 0
    else:
        elem = elem[0]  # 검색 결과 있음

    cntText = elem.text.split()
    cntText = cntText[2].strip()  # 검색 결과 수

    if cntText == '1':
        return 1, getBookInfo(1)
    else:
        str = f'총 {cntText} 건의 검색 결과가 존재합니다.'
        if int(cntText.replace(',', '')) < 6:
            str += '\n검색된 결과는 다음과 같습니다.'
            return 2, str, getTopItems()
        else:
            str += '\n상위 5개의 검색 결과는 다음과 같습니다.'
            return 3, str, getTopItems()
